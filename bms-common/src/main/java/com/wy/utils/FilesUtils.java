...
/**
 * 本项目所有上传文件的接口都必须是文件先上传成功,文件上传到本地,
 * 然后将上传成功的文件数据插入到数据库表中,之后再进行其他操作,方便统一管理上传文件
 * @attention 为方便文件备份或转移,上传的文件都以当天文件夹区分,文件命名方式为:
 *            32位uuid_yyyyMMdd
 * @author paradiseWy
 */
public class FilesUtils {
	...
	public static void main(String[] args) {
		System.out.println(DateUtils.format(new Date(), DateEnum.DATE));
	}
	...
	public static String saveFile(MultipartFile file) {
		...
		String localName = MessageFormat.format(FILE_FORMATTER, R.UU32(),
				DateUtils.format(new Date(), "yyyyMMdd"));
		localName = StrUtils.isBlank(extension) ? localName
				: MessageFormat.format("{0}.{1}", localName, extension);
		try {
